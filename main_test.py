...
def Extrai_Dados(site, produto, tipo_produto):
    ...
    lista_preco = [
    f'{int(digits[:-2]):,}.{digits[-2:]}' 
    for digits in [re.sub(r"\D", "", texto) for texto in lista_preco]]
    ...
    lista_tipo_produto = [tipo_produto] * len(lista_produtos)

    # Combinacão em um único array de tuplas
    dados_combinados = list(zip( lista_produtos, lista_preco, lista_tipo_produto, lista_site, lista_links, lista_data_atual))
...
if __name__ == "__main__":
    Extrai_Dados("Kabum", "Placa de Vídeo RTX 4070", "Placa de Video")
